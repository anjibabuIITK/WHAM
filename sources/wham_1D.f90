...
if(parent)then
  ...
  !read number of cv, number of umbrella, Temperature in K
...
STOP 'the code is only expecting ncv=1'
end if
end if
...
if (parent) then
   ...
   do i_umbr=1, umbr_n 
      ...
      read(2,'(a)') cvfile
      write(*,*)'CVFILE',cvfile
      !reads mean position of umbrella, k_value of umbrella, and number of points in umbreall
      read(2,*) umbr_mean(i_umbr),umbr_k(i_umbr),nmax(i_umbr) 
      umbr_k(i_umbr)=umbr_k(i_umbr)*kj_to_kcal !converted in kcal
      !reads the probability written in prob file for each umbrell_
      ...
      do i_s1=1,nbin1 !US
         read(3,*)dummy_1,biased_prob(i_s1,i_umbr)
      end do
   enddo
end if
...
!prints pmf 
 ...
 real*8 :: s1, s2, kt, dum,dum1
 logical:: parent
 real*8, allocatable :: prob1(:)
 character (len=50):: f2,f3
...
! open( unit =8 , file = f3, status =  'unknown' )
 do i_s1=1,nbin1 !US cv
  ...
  !   s2=DFLOAT(i_s2-1)*grid0(3,2)+grid0(1,2)
     ...
     write(7,'(3E16.8)') s1, dum, prob1(i_s1)
!  enddo
!  write(7,*)
 enddo
